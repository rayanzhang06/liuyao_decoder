...
class KimiClient(BaseLLMClient):
    """Moonshot Kimi 客户端（使用 OpenAI 兼容接口）"""
    ...
    def chat(self,
        ...
        logger.info(f"调用 Moonshot Kimi API: model={model}, messages={len(messages)}")
        ...
        try:
            ...
            logger.info(f"Moonshot Kimi API 响应成功: 耗时 {elapsed_time:.2f}s")
            ...
            logger.error(f"Moonshot Kimi API 调用失败: {e}")
            raise
    ...
    def count_tokens(self, text: str) -> int:
        ...
        Moonshot Kimi 使用与 OpenAI 类似的 token 计算方式
        ...
        try:
            ...
            # Kimi 使用 cl100k_base 编码器（与 GPT-4 相同）
