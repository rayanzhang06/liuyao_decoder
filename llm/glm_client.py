...
class GLMClient(BaseLLMClient):
    ...
    def chat(self,
        ...
        logger.info(f"调用 Zhipu GLM API: model={model}, messages={len(messages)}")
        ...
        try:
            ...
            logger.info(f"Zhipu GLM API 响应成功: 耗时 {elapsed_time:.2f}s")
            ...
            logger.error(f"Zhipu GLM API 调用失败: {e}")
            raise
    ...
    def count_tokens(self, text: str) -> int:
        ...
        """
        try:
            # ZhipuAI 没有提供直接的 token 计数方法
            # 使用粗略估算：中文 1 token ≈ 0.75 个字符，英文 1 token ≈ 4 个字符
            chinese_chars = sum(1 for c in text if '\u4e00' <= c <= '\u9fff')
            other_chars = len(text) - chinese_chars
            return int(chinese_chars / 0.75 + other_chars / 4)
        except Exception as e:
            logger.warning(f"Token 计算失败: {e}")
            return len(text)  # 最坏情况返回字符数
